...
export class Orbis {
    ...
    async connect(provider) {
        ...
        try {
            ...
            await AsyncStorage.setItem('ceramic-session', sessionString)
        } catch (e) {
        ...
        let _userAuthSig = await AsyncStorage.getItem('lit-auth-signature-' + addresses[0])
        if (!_userAuthSig || _userAuthSig == '' || _userAuthSig == undefined) {
            ...
            await AsyncStorage.setItem('lit-auth-signature', _userAuthSig)
        }
    ...
    async logout() {
        try {
            await AsyncStorage.removeItem('ceramic-session')
            return {
                status: 200,
    ...
    async updateConversation(stream_id, content) {
        ...
        let result = await this.updateTileDocument(
            stream_id,
            _content,
            ['orbis', 'conversation'],
