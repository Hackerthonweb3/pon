...
export class Orbis {
    ...
    async connect(provider, lit = true) {
        ...
        if (lit === true) {
            ...
            if (!_userAuthSig || _userAuthSig === '' || _userAuthSig === undefined) {
                ...
                /**
                 * User is already connected, save current accoutn signature in lit-auth-signature object for easy
                 * retrieval
                 */
    ...
    /**
     * Automatically reconnects to a session stored in localStorage, returns false if there isn't any session in
     * localStorage
     */
    async isConnected() {
        await this.ceramic
    ...
    async testConnectSolana() {
        ...
        /**
         * Step 3 bis: Store session in localStorage to re-use try { const sessionString = this.session.serialize()
         * localStorage.setItem("ceramic-session", sessionString); } catch(e) { console.log("Error creating
         * sessionString: " + e); }
         */
    ...
    async setNotificationsReadTime(type, timestamp, context = null) {
        ...
        if (context) {
            ...
            result = await this.createTileDocument(
                ...
                ['orbis', 'settings', 'notifications', type],
    ...
    /**
     * NOT AVAILABLE FOR NOW: Users can create or update a new context such as a group or a channel within a group to
     * organize the posts being shared
     */
    async createContext() {}
    async updateContext() {}
    ...
    /**
     * Retrieve posts shared in a specific context or by a specific user Returns an array of posts in the `data` field
     * or an `error`.
     */
    async getPosts(options, page = 0) {
        ...
        if (options?.algorithm) {
                    ...
                    if (options && options.context) {
                        query = this.api
                            .rpc('all_did_master_posts_with_context', {
                                post_did: options?.did,
                                post_context: options.context,
                            })
                            .range(page * 50, (page + 1) * 50 - 1)
                    } else {
                        query = this.api
                            .rpc('all_did_master_posts', { post_did: options?.did })
                            .range(page * 50, (page + 1) * 50 - 1)
                    }
                    break
                case 'all-context-master-posts':
                    query = this.api
                        .rpc('all_context_master_posts', { post_context: options?.context })
                        .range(page * 50, (page + 1) * 50 - 1)
                    break
            ...
            query = this.api
                .from('orbis_v_posts')
                .select()
                .eq('context', options.context)
                .eq('creator', options.did)
                .range(page * 50, (page + 1) * 50 - 1)
                .order('timestamp', { ascending: false })
    ...
    async getReaction(post_id, did) {
        let { data, error, status } = await this.api
    ...
    async getGroupMembers(group_id) {
        let { data, error, status } = await this.api
            .from('orbis_v_members')
            .select()
            .match({ group_id: group_id, active: 'true' })
    ...
    async getIsGroupMember(group_id, did) {
        let res = false
        let { data, error, status } = await this.api
            .from('orbis_group_members')
            .select('stream_id')
            .match({ group_id: group_id, did: did, active: 'true' })
    ...
    async getProfileGroups(did) {
        let { data, error, status } = await this.api
            .from('orbis_user_groups')
            .select()
            .match({ did: did, active: 'true' })
    ...
    async getIsFollowing(did_following, did_followed) {
        ...
        let { data, error, status } = await this.api
            .from('orbis_v_followers')
            .select()
            .match({ did_following: did_following, did_followed: did_followed, active: 'true' })
